...
const defaultFieldFormatter = (field:any) => field === undefined ? 'no data' : JSON.stringify(field);
...
const statsFields:{[key in keyof Stats_statistics]:statFields[]} = {
    ...
    backlogLength: [
      {
        title: "Backlog",
        goodThreshold: (length: number, blockDuration: number) => {return (length < 1000) || (length >= 1000 && (blockDuration / 1000000000) <= 1) },
      }
    ],
    tradesPerSecond: [
      {
    ...
    blockDuration: [
      {
        ...
        goodThreshold: (blockDuration: number) => blockDuration > 0 && blockDuration <= 2000000000,
      }
    ],
    status: [
      {
        ...
        goodThreshold: (status: string) => (status === 'CONNECTED' || status === 'CHAIN_STATUS_CONNECTED'),
      }
    ],
    totalPeers: [
      {
    ...
    vegaTime: [
      {
        ...
        goodThreshold: (time: Date) => { let diff = new Date().getTime() - new Date(time).getTime(); return diff > 0 && diff < 5000; },
      }
    ],
    appVersion: [
      {
    ...
    upTime: [
      {
        ...
        formatter: (t: string) => {
          if (!t)  { return }
          const secSinceStart = (new Date().getTime() - new Date(t).getTime()) / 1000
...
const renderStats = (statKey: keyof Stats_statistics, statData: string) => {
    if (statKey in statsFields) {
      statsFields[statKey].map((s:any) => {
        return (
          <div>
            <h3>{s.title}</h3>
            <div className='value'>{s.formatter ? s.formatter(statData) : defaultFieldFormatter(statData)}</div>
            {s.goodThreshold ? <div className='threshold'>Good threshold met: {s.goodThreshold(statData)}</div> : null}
          </div>)
      });
    }
  }
...
export const Stats = () => {
  ...
  return (
    <div>
      <h2>Network Stats</h2>
      <div>{Object.keys(statsFields).map((statKey) => renderStats(statKey, returnedStatistics[statKey]))}</div>
    </div>
  );
};
